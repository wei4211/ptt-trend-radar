...
async def trigger_scrape(
    ...
    if board:
        background_tasks.add_task(run_scrape_and_analyze, db, board, 3)
    else:
        background_tasks.add_task(_scrape_all, db)

    return {"status": "scraping started", "board": board or "all"}
...
async def trigger_scrape_sync(
    ...
    """Synchronous scrape (blocks until done). Good for initial setup."""
    if board and board not in settings.SUPPORTED_BOARDS:
        raise HTTPException(400, f"Board must be one of {settings.SUPPORTED_BOARDS}")
    ...
    results = []
    for b in boards:
        result = await run_scrape_and_analyze(db, b, pages=3)
        results.append(result)
