    ...
    <div className="p-8 space-y-6">
      ...
      {!sentLoading && sentiment && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="正面" value={`${sentiment.positive}%`} icon="😊" accent="emerald" />
          <StatCard title="中性" value={`${sentiment.neutral}%`} icon="😐" accent="sky" />
          <StatCard title="負面" value={`${sentiment.negative}%`} icon="😟" accent="rose" />
          <StatCard
            title="平均分數"
            value={sentiment.avg_score.toFixed(3)}
            icon="📊"
            subtitle={`共 ${sentiment.total} 篇`}
            accent="amber"
          />
        </div>
      )}
