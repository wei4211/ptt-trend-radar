...
function TopicDetail({ topic, board, color }: { topic: Topic; board: Board; color: string }) {
  ...
  const handleGenerateSummary = () => {
    generate(topic.id, board === "all" ? "Gossiping" : board, (s) => setSummary(s));
  };
      ...
      <div className="glass rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          >
            ...
            {summaryLoading ? "生成中..." : summary ? "重新生成" : "用 Gemini 生成"}
          </button>
        </div>
        {summary ? (
          <p className="text-sm text-slate-300 leading-relaxed">{summary}</p>
        ) : (
          <p className="text-sm text-slate-500 italic">點擊右上角按鈕，讓 Gemini 自動摘要這個主題的討論內容</p>
        )}
      </div>
            ...
            {selected ? (
              ...
              <div className="glass rounded-xl h-full flex items-center justify-center py-24">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-slate-500 text-sm">點選左側主題查看詳情</p>
                </div>
              </div>
            )}
          </div>
